...
CALIBRATION_API bool R_T2H(const cv::Mat &R, const cv::Mat &T, cv::Mat &H_out);

CALIBRATION_API bool H2R_T(const cv::Mat &H, cv::Mat &R, cv::Mat &T);

CALIBRATION_API bool isRotationMatrix(const cv::Mat &R);

// bool isRotationMatrix(const Eigen::Matrix3d& R);

CALIBRATION_API bool eulerToRotationMatrix(const cv::Vec3d &euler_angles,
...
CALIBRATION_API bool quaternionToRotationMatrix(const cv::Vec4d &q, cv::Mat &R_output, int dtype = CV_64F);

CALIBRATION_API cv::Mat poseToHomogeneousMatrix(
    const cv::Mat &pose_data, const std::string &rotation_order = "ZYX", int dtype = CV_64F);

CALIBRATION_API cv::Mat inverseHomogeneous(const cv::Mat &H);

CALIBRATION_API bool eulerToRotationMatrix(const cv::Mat &euler_angles,
                                                cv::Mat &R, const std::string &rotation_order = "ZYX");
CALIBRATION_API bool poseToRT_deg(const cv::Mat &pose, cv::Mat &R, cv::Mat &t);
CALIBRATION_API bool poseToRT_rad(const cv::Mat &pose, cv::Mat &R, cv::Mat &t);

// Save cv::Mat to project root's "output" directory as JSON.
// Only file name is required. Default key is "T_base_camera".
CALIBRATION_API bool saveMatrixToJsonInOutput(
    const cv::Mat &mat,
